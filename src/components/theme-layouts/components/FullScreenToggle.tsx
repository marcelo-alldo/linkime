...
function HeaderFullScreenToggle(props: HeaderFullScreenToggleProps) {
  ...
  useEnhancedEffect(() => {
    document.onfullscreenchange = () => setIsFullScreen((document as FullScreenDocumentType)[getBrowserFullscreenElementProp()] != null);
  ...
  function toggleFullScreen() {
    ...
    if (doc.fullscreenElement || doc.webkitFullscreenElement || doc.mozFullScreenElement) {
      closeFullscreen();
    } else if (elem.requestFullscreen || elem.mozRequestFullScreen || elem.webkitRequestFullscreen || elem.msRequestFullscreen) {
      openFullscreen();
    }
  }
  ...
  return (
    <Tooltip title="Fullscreen toggle" placement="bottom">
      <IconButton onClick={toggleFullScreen} className={clsx('border border-divider', className, isFullScreen && 'text-red-500')}>
