...
function FuseShortcuts(props: FuseShortcutsProps) {
  ...
  useEffect(() => {
    const _shortcutItems = shortcuts ? shortcuts.map((id) => _.find(navigation, { id })) : ([] as FuseFlatNavItemType[]);
  ...
  return (
    <Box className={clsx('flex shrink overflow-hidden', variant === 'vertical' ? 'flex-col' : '', className)}>
      {useMemo(() => {
        return (
          <Box className={clsx('flex flex-1 items-center border rounded-lg', variant === 'vertical' ? 'flex-col' : 'max-h-9')}>
            {shortcutItems.map(
              (_item) =>
                _item && (
      ...
      >
        ...
        {useMemo(() => {
          ...
          return shortcutItems.map(
            (_item) => _item && <ShortcutMenuItem shortcuts={shortcuts} key={_item.id} item={_item} onToggle={() => toggleInShortcuts(_item.id)} />,
          );
...
function ShortcutMenuItem(props: { shortcuts: FuseShortcutsProps['shortcuts']; item: FuseFlatNavItemType; onToggle: (T: string) => void }) {
    ...
    <Link to={item.url || ''} role="button">
      <MenuItem key={item.id}>
        <ListItemIcon className="min-w-9">
          {item.icon ? <FuseSvgIcon>{item.icon}</FuseSvgIcon> : <span className="text-center text-2xl font-semibold uppercase">{item.title[0]}</span>}
        </ListItemIcon>
        ...
        <IconButton
          ...
          size="large"
        >
          <FuseSvgIcon color="action">{shortcuts.includes(item.id) ? 'heroicons-solid:star' : 'heroicons-outline:star'}</FuseSvgIcon>
        </IconButton>
      </MenuItem>
    </Link>
  );
