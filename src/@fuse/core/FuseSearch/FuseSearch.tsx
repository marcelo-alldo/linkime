      ...
      {variant === 'basic' ? (
        ...
        <>
          <TextField
            ...
            {...other}
          />
          <FuseSvgIcon className="pointer-events-none absolute top-0 h-9 w-9 p-2 ltr:right-0 rtl:left-0" color="action" size={20}>
...
function FuseSearch(props: FuseSearchProps) {
  ...
  function handleSuggestionSelected(event: React.FormEvent<unknown>, { suggestion }: { suggestion: FuseFlatNavItemType }) {
  ...
  function handleClickAway(event: MouseEvent | TouchEvent) {
    if (state.opened && (!suggestionsNode.current || !(event.target instanceof Node && suggestionsNode.current.contains(event.target)))) {
      hideSearch();
    }
  }
        ...
        <div className={clsx('flex w-full items-center', className)} ref={popperNode}>
          <Autosuggest
            ...
            renderSuggestionsContainer={(options) => {
              ...
              return (
                <Popper anchorEl={popperNode.current} open={Boolean(options.children) || state.noSuggestions} className="z-9999">
