...
function FuseExample(props: FuseExampleProps) {
  ...
  return (
    <Card className={clsx(className, 'shadow-sm not-prose')}>
      <Box
        sx={{
          backgroundColor: (theme) => darken(theme.palette.background.paper, theme.palette.mode === 'light' ? 0.02 : 0.2),
        }}
      >
        <Tabs
          classes={{
        ...
        >
          ...
          {raw && <Tab classes={{ root: 'min-w-16' }} icon={<FuseSvgIcon>heroicons-outline:code-bracket</FuseSvgIcon>} />}
        </Tabs>
      </Box>
