    ...
    <Root id="fuse-layout" className="flex flex-auto w-full" config={config}>
      ...
      <div className="flex min-w-0 flex-auto flex-col">
        <main id="fuse-main" className="relative flex min-h-full min-w-0 flex-auto flex-col">
          {config.navbar.display && <NavbarWrapperLayout2 className={clsx(config.navbar.style === 'fixed' && 'sticky top-0 z-50')} />}

          {config.toolbar.display && (
            <ToolbarLayout2
              className={clsx(config.toolbar.style === 'fixed' && 'sticky top-0', config.toolbar.position === 'above' && 'z-40 order-first')}
            />
          )}
